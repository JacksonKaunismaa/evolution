...
TensorWithShape = Tuple[Tensor, Tuple[int]]

class QuantizedData:
    def __init__(self, x: Union[List[TensorWithShape], TensorWithShape]):
        self.is_list = isinstance(x, list)
        if self.is_list:
            self.data = [x_val[0] for x_val in x]
            self.shape = [x_val[1] for x_val in x]
        else:
            self.data, self.shape = x
            
    def _dequantize(self, y: Tensor, shape: Tuple[int], map_location: torch.device) -> Tensor:
        return y.dequantize().view(shape).to(map_location)

    def dequantize(self, map_location='cuda') -> Union[List[Tensor], Tensor]:
        if self.is_list:
            return [self._dequantize(y_val, y_shape, map_location) for y_val, y_shape in zip(self.data, self.shape)]
        return self._dequantize(self.data, self.shape, map_location)
            

def _quantize(x: Tensor, map_location='cpu') -> TensorWithShape:
...
def quantize(x: Union[List[Tensor], Tensor], map_location='cpu') -> QuantizedData:
    if isinstance(x, list):
        qdata = [_quantize(x_val, map_location) for x_val in x]
    else:
